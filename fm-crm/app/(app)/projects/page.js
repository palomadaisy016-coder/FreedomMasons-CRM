...
export default function ChatsPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-140px)] border border-line rounded-lg overflow-hidden">
      {/* Conversation list: full width on mobile when no chat open, fixed sidebar on desktop always */}
      <div
        className={`${
          active ? "hidden" : "flex"
        } md:flex w-full md:w-64 border-r border-line bg-paper flex-col shrink-0`}
      >
      ...
      {/* Thread panel: full width on mobile when a chat is open, otherwise hidden on mobile */}
      <div className={`${active ? "flex" : "hidden"} md:flex flex-1 flex-col min-w-0`}>
        {!active ? (
          <div className="flex-1 items-center justify-center text-sm text-muted hidden md:flex">
            Select a conversation, or start a new one.
          </div>
        ) : (
          <>
            <div className="px-4 py-3 border-b border-line font-medium text-sm text-ink flex items-center gap-2">
              <button
                onClick={() => setActive(null)}
                className="md:hidden -ml-1 p-1 rounded hover:bg-paper shrink-0"
              ...
              {thread.map((m) => {
                ...
                return (
                  <div key={m.id} className={`flex flex-col ${mine ? "items-end" : "items-start"}`}>
                    <div
                      className={`max-w-[85%] sm:max-w-sm rounded-lg px-3 py-2 text-sm ${
